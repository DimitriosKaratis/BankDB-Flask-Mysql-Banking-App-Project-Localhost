...
db_config = {
    'host': 'localhost',
    'user': 'root',
    'password': '1234', # User's specific password
    'database': 'BankDB'
}
...
def login():
    ...
    if request.method == 'POST':
        ...
        password = request.form['password'] # (no password field)
...
@login_required
def dashboard():
    """Displays customer dashboard with accounts, cards, loans, net worth etc."""
...
def pay_credit():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Net Account Change: +Amount - 2*Amount = -Amount 
            # Net Card Change: +Amount
            tid_2 = tid_1 + 1
            cursor.execute("INSERT INTO transaction (TransactionID, Date, Time, Amount) VALUES (%s, %s, %s, %s)", 
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if action == 'update_address':
                ...
                # Check if email exists to avoid duplicate errors 
        ...
        # 2. Address: Fetch from customer table 
